  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 sm:gap-2 rounded-xl bg-[#942E3A] px-2.5 py-2 sm:px-4 sm:py-3 text-[11px] sm:text-xs font-bold text-[#FFF9EB] shadow-xs transition hover:bg-[#7e2732] shrink-0"
      >
        <UserPlus className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#D8B46A] shrink-0" />
      ...
      {open && (
        ...
        >
          <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-[#D8B46A]/35 bg-[#FFFDFC] p-5 shadow-2xl sm:p-7 text-right">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#D8B46A]">
                  {isRtl ? "صلاحيات الوصول" : "Workspace access"}
                </p>
                <h2 className="mt-1 font-playfair text-2xl font-black text-[#942E3A]">
                  {t("team.addMember")}
                </h2>
                <p className="mt-1 text-xs text-[#6B1F2A]/60">
                  {isRtl ? "إنشاء حساب مسؤول جديد للوحة تحكم دي روما." : "Create a new account with access to the DeRoma back office."}
                </p>
              </div>
              <button
                type="button"
            ...
            <form action={createTeamMemberAction} className="mt-6 space-y-3">
              ...
              <label className="block">
                ...
                <input
                  ...
                  type="email"
                  className={`admin-input ${isRtl ? "text-right" : "text-left"}`}
                  placeholder="name@deroma.com"
                />
              </label>

              <label className="block">
                <span className="field-label">{isRtl ? "كلمة السر المؤقتة *" : "Temporary password *"}</span>
