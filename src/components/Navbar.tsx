    ...
    <div className={cn(
      ...
      <div className="mx-auto flex w-full max-w-[1320px] items-center justify-between gap-2 sm:gap-3">
        ...
        <header className="relative pointer-events-auto flex min-w-0 flex-1 h-12 sm:h-12 items-center justify-between rounded-[1.35rem] sm:rounded-full bg-[#942E3A]/95 text-white backdrop-blur-xl px-3 sm:px-5 shadow-xl border border-white/20 transition-all duration-300">
            ...
            {showSearchInput && (
              <form onSubmit={handleSearchSubmit}>
                <input
                  ...
                  className="mr-1.5 rtl:mr-0 rtl:ml-1.5 text-[11px] bg-white/10 border border-white/20 rounded-full px-2.5 py-0.5 text-white placeholder-stone-300 focus:outline-none focus:ring-1 focus:ring-[#942E3A] transition-all duration-150 w-36"
                  autoFocus
                />
              </form>
            )}
          ...
          {/* Mobile search overlay container - Ultra lightweight pure CSS */}
          <div
            className={cn(
              "absolute right-2 rtl:right-auto rtl:left-2 top-1/2 -translate-y-1/2 w-48 h-8 rounded-full bg-[#942E3A] text-white border border-white/20 px-2.5 shadow-xl lg:hidden flex items-center gap-1.5 z-50 transition-all duration-150 ease-out transform-gpu",
              showMobileSearch
