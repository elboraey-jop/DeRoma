...
export function AdminSupplierPicker({
  suppliers,
  value,
  onChange,
  onAddNew,
      ...
      {open && (
        <div className="absolute left-0 top-[calc(100%+8px)] z-40 w-full min-w-[240px] overflow-hidden rounded-2xl border border-[#D8B46A]/45 bg-[#FFF9EB] p-2 shadow-[0_18px_40px_rgba(67,25,31,0.2)]">
          ...
          <div
            ...
            className="hide-scrollbar mt-2 max-h-56 space-y-1 overflow-y-auto overscroll-contain"
          >
...
function PlusIcon() {
  return <span className="text-base leading-none text-[#D8B46A]">+</span>;
}
...
export function AdminProductPicker({
  ...
  const categories = [
    "all",
    ...Array.from(new Set(variants.map((variant) => variant.category))),
  ];
  const filtered = useMemo(
    () =>
      variants.filter((variant) => {
        ...
        return (
          (category === "all" || variant.category === category) &&
      ...
      >
        <span className="flex min-w-0 flex-1 items-center gap-3">
          {selected?.image ? (
            ...
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#F2DFC0] text-[#D8B46A]">
            ...
            {filtered.map((variant) => (
              ...
              >
                {variant.image ? (
                  ...
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-white text-[#D8B46A]">
                ...
                {variant.id === value && (
                  <Check className="h-4 w-4 shrink-0 text-[#D8B46A]" />
                )}
              </button>
            ))}
            {!filtered.length && (
...
export function AdminCatalogProductPicker({
  products,
  value,
  onChange,
}: {
