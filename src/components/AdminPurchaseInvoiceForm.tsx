...
export default function AdminPurchaseInvoiceForm({
  ...
  return (
    <form action={createPurchaseInvoiceAction} className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_0.7fr]">
        <section className="rounded-3xl border border-[#942E3A]/10 bg-white p-5 shadow-sm sm:p-7">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#D8B46A]">Invoice information</p>
          <h2 className="mt-1 font-playfair text-xl font-bold">Supplier & invoice</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="sm:col-span-2">
              ...
              <textarea name="notes" rows={4} className="admin-input resize-y" placeholder="Add any notes about this purchase..." />
            </label>
          </div>
        </section>
      ...
      <section className="rounded-3xl border border-[#942E3A]/10 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#D8B46A]">Stock receipt</p>
            <h2 className="mt-1 font-playfair text-xl font-bold">Products on this invoice</h2>
            <p className="mt-1 text-xs text-[#6B1F2A]/60">The summary updates instantly as you build the invoice.</p>
          </div>
          <Link href="/admin/products/new?fromInvoice=1" className="text-center text-[10px] font-bold text-[#942E3A] underline">Need a new product? Create it first</Link>
        </div>

        <div className="mt-5 flex flex-col gap-2 sm:flex-row">
          <AdminProductPicker variants={variants as ProcurementVariant[]} value={selectedVariant} onChange={setSelectedVariant} />
          <button type="button" onClick={addLine} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[#D8B46A] px-4 py-3 text-xs font-bold text-[#942E3A]"><Plus className="h-4 w-4" /> Add product</button>
        </div>

        <div className="mt-4 space-y-3">
          {lines.map((line) => (
            <div key={line.id} className="rounded-2xl bg-[#FFF9EB]/70 p-4">
              <div className="flex items-start justify-between gap-3">
                <div><p className="font-bold text-[#942E3A]">{line.productName}</p><p className="mt-1 text-[10px] text-[#6B1F2A]/60">{line.label}</p></div>
                <button type="button" onClick={() => setLines((current) => current.filter((item) => item.id !== line.id))} className="text-red-600" aria-label="Remove product"><Trash2 className="h-4 w-4" /></button>
              </div>
          ...
          {!lines.length && <div className="rounded-2xl border border-dashed border-[#D8B46A]/60 py-12 text-center text-xs text-[#6B1F2A]/60">Choose a product variant above to start the invoice.</div>}
        </div>
      </section>

      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <Link href="/admin/suppliers" className="rounded-xl border border-[#942E3A]/15 bg-white px-5 py-3 text-center text-xs font-bold text-[#942E3A]">Cancel</Link>
        <button type="submit" disabled={!lines.length} className="rounded-xl bg-[#942E3A] px-6 py-3 text-xs font-bold text-[#FFF9EB] disabled:cursor-not-allowed disabled:opacity-40">Save invoice & receive stock</button>
      </div>
    </form>
  );
}
