  ...
  const chooseStatus = (nextStatus: string) => {
    ...
    if (nextStatus !== status) {
      toast.success(
        ...
        lang === "ar" ? "تم تحديث الطلب" : "ORDER UPDATED"
      );
          ...
          <div className="mt-1 space-y-0.5">
            {options.map((option) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-xs font-bold transition-colors ${option === status ? "bg-[#942E3A] text-[#FFF9EB]" : "text-[#942E3A] hover:bg-[#F2DFC0]"}`}
              >
                <span>{getStatusLabel(option, lang)}</span>
