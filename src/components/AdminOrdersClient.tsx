...
const statuses = ["all", "pending", "pending_payment", "paid", "confirmed", "preparing", "shipped", "delivered", "cancelled", "returned"];
...
export default function AdminOrdersClient({ orders }: { orders: AdminOrderRow[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-3xl border border-[#942E3A]/10 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex gap-1 overflow-x-auto pb-1">{statuses.map((item) => <button key={item} type="button" onClick={() => setStatus(item)} className={`whitespace-nowrap rounded-full px-3 py-2 text-[10px] font-bold capitalize ${status === item ? "bg-[#942E3A] text-[#FFF9EB]" : "bg-[#FFF9EB] text-[#942E3A]/70"}`}>{item === "all" ? "All orders" : getStatusLabel(item)}</button>)}</div><label className="relative block lg:w-72"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#D8B46A]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search order, customer, phone" className="w-full rounded-xl border border-[#942E3A]/10 bg-[#FFF9EB]/60 py-2.5 pl-9 pr-3 text-xs outline-none focus:border-[#942E3A]" /></label></div>
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="border-b border-[#942E3A]/10 text-[10px] uppercase tracking-wide text-[#6B1F2A]/55"><tr><th className="pb-3">Order</th><th className="pb-3">Customer</th><th className="pb-3">Date</th><th className="pb-3">Status</th><th className="pb-3 text-right">Total</th><th className="pb-3 text-right">Open</th></tr></thead><tbody className="divide-y divide-[#942E3A]/8">{filtered.map((order) => <tr key={order.id}><td className="py-3 font-bold text-[#942E3A]"><Link href={`/admin/orders/${order.id}`} className="hover:underline">{order.orderNumber}</Link></td><td className="py-3"><Link href={`/admin/customers/${encodeURIComponent(order.customerPhone)}`} className="font-bold text-[#942E3A] hover:underline">{order.customerName}</Link><p className="text-[10px] text-[#6B1F2A]/60">{order.customerPhone}</p></td><td className="py-3 text-[#6B1F2A]/65">{new Date(order.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</td><td className="py-3"><AdminStatusSelect orderId={order.id} status={order.status} paymentMethod={order.paymentMethod} /></td><td className="py-3 text-right font-bold text-[#942E3A]">{formatCurrency(order.totalPrice)}</td><td className="py-3 text-right"><Link href={`/admin/orders/${order.id}`} className="inline-flex items-center gap-1 font-bold text-[#942E3A] hover:underline"><Eye className="h-3.5 w-3.5" /> View</Link></td></tr>)}</tbody></table>{filtered.length === 0 && <div className="flex flex-col items-center justify-center py-12 text-center"><ClipboardList className="h-7 w-7 text-[#D8B46A]" /><p className="mt-2 text-sm font-bold">No orders found</p><p className="mt-1 text-xs text-[#6B1F2A]/60">New orders will appear here.</p></div>}</div>
      </div>
    </div>
  );
}
