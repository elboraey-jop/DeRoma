...
export default async function CustomersPage() {
  ...
  try {
    ...
    for (const order of orders) { const current = map.get(order.customerPhone); if (current) { current.orders += 1; current.spent += Number(order.totalPrice); current.lastOrder ??= order.createdAt; } else map.set(order.customerPhone, { id: "", name: order.customerName, email: null, phone: order.customerPhone, phone2: null, governorate: order.governorate, city: order.city, address: order.address, notes: null, createdAt: order.createdAt, updatedAt: order.createdAt, orders: 1, spent: Number(order.totalPrice), lastOrder: order.createdAt }); }
